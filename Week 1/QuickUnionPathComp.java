...
EXTRA Note - Path compression - a way to flatten the tree
This is the same as normal quick union but with one addition:
- Path compression: in root method, after going up tree, set the root of every node examined
to the topmost root - constant extra cost

- Less costly method(implemented here) - make every node point to its grandparent node
    - Doesn't completely flatten the tree like the other method, but it is almost as good with less cost
 */
...
public class QuickUnionUF{
    ...
    public int findTopRoot(int index){
        ...
        while(id[index] != index){
            //Path compression = make the immediate root of this node that of its grandparent node
